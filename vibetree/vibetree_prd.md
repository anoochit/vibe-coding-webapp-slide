...
The code for the project will be created and maintained in the current directory.
...
service cloud.firestore {
  match /databases/{database}/documents {
    // 1. Users collection rules (VibeTree Users)
    match /users/{userId} {
      allow read: if true; // Publicly readable profiles
      allow create, delete: if request.auth != null && (request.auth.uid == userId );
      allow update: if (request.auth != null && (request.auth.uid == userId ))
                    || (request.resource.data.diff(resource.data).affectedKeys().hasOnly(['links', 'updatedAt'])); // Public anonymous clicks
    }
...
*   **Emulator Enforcement**: During local development (when `process.env.NODE_ENV === 'development'`), the client-side Firebase SDK must automatically connect to the local Firebase Emulator Suite instead of the production Firebase project.
*   **Emulator Services**: Connection must be established to:
    *   **Firebase Authentication**: Port `9099`
    *   **Cloud Firestore**: Port `8080`
    *   **Firebase Storage**: Port `9199`
*   **Data Integrity**: This guarantees local tests, seed data, and rules debugging can happen completely isolated, preventing pollution of any production database or storage bucket.
...
* [x] **Task 1.1:** Initialize Next.js (App Router) app in the current directory with Tailwind CSS, `lucide-react`, and `chart.js`.
* [x] **Task 1.2:** Configure tailwind setup, primary styles, and core CSS variables supporting the 4 dynamic themes.
* [x] **Task 1.3:** Setup routing structure (`/`, `/dashboard`, `/[username]`) and configure Firebase SDK. Integrate automatic detection for local development mode (`process.env.NODE_ENV === 'development'`) to seamlessly connect to the local Firebase Emulator Suite (Auth, Firestore, Storage). Fallback to standard config with an elegant environment-variable setup check (directing to a setup helper screen if config keys are missing in production).
* [x] **Task 1.4:** Create stunning landing page with **Firebase Authentication** Email & Password/Google Authentication UI.
* [x] **Task 1.5 [DoD / Verification]:** Start local Firebase Emulators. Verify that visiting the site in development mode connects successfully to the local Emulators (Auth, Firestore, Storage) and does not write to production. Verify that in production mode, visiting without `.env` variables successfully intercepts navigation and displays the setup helper screen.

### 🟦 Milestone 2: Split-Panel Builder & Live Preview (P0)
* [x] **Task 2.1:** Design split-panel dashboard (Left: control panel with forms; Right: glassmorphic floating mobile-phone frame).
* [x] **Task 2.2:** Build Profile editor (Avatar upload, bio text, username, theme selector) with instant live preview.
* [x] **Task 2.3:** Build Dynamic Link List builder (add, edit title, edit URL, delete links) with real-time reactive updating.
* [x] **Task 2.4:** Implement automatic social URL recognition to auto-assign premium brand icons.
* [x] **Task 2.5 [DoD / Verification]:** Verify that adding, updating, or deleting links and profile fields instantly updates the visual representation in the mock phone frame in real-time. Test multiple social links (YouTube, X, GitHub) and verify icons appear correctly.

### 🔥 Milestone 3: Firestore Synchronization & Real-Time Sync (P0)
* [x] **Task 3.1:** Map profile state and link edits to Firestore `.set()` or `.update()` actions using the `users` and `usernames` schema.
* [x] **Task 3.2:** Implement real-time listener updates using `onSnapshot` for dashboard editing.
* [x] **Task 3.3:** Design and implement the public profile page (`/[username]`) which loads user profiles from Firestore in real-time.
* [x] **Task 3.4 [DoD / Verification]:** Open two parallel browser windows (Dashboard & Public Profile). Edit links in the dashboard and verify that changes propagate to the public profile instantly without page reload. Test duplicate usernames and verify uniqueness protection.

### 📷 Milestone 4: Storage Uploads & Custom Drag-and-Drop (P1)
* [x] **Task 4.1:** Embed custom avatar file input. Upload chosen image to Firebase Storage and save the returned download URL to Firestore.
* [x] **Task 4.2:** Implement standard HTML5 draggable elements on link rows in the builder panel.
* [x] **Task 4.3:** Rearrange the list array on dropping a link, and sync the updated ordering directly to Firestore.
* [x] **Task 4.4 [DoD / Verification]:** Confirm drag-and-drop handles are responsive and reorder state immediately. Test uploading files larger than 2MB or non-image formats, checking that client-side validations block the upload and render an aesthetic warning toast.

### 🎨 Milestone 5: Styling Themes, Click Analytics & Deployment (P1/P2)
* [x] **Task 5.1:** Integrate the 4 gorgeous themes using custom tailwind / CSS variables.
* [x] **Task 5.2:** Add click metric event tracking. Increment Firestore metric counters when preview links are clicked.
* [x] **Task 5.3:** Create interactive dashboard charts tracking click performance using Chart.js.
* [x] **Task 5.4:** Prepare deployment configurations and run `firebase deploy`.
* [x] **Task 5.4b:** Create local `firestore.rules` and `storage.rules` files matching our security specifications in Part 1.7, and configure `firebase.json` to deploy these rules.
* [x] **Task 5.5 [DoD / Verification]:** Click on active public links, then verify click counts increment correctly inside Firestore subcollections. Check that the Chart.js widgets render these clicks accurately over a timeline. Verify the fully deployed app renders correctly on mobile devices.
