import React from "react";
import { 
  Globe, 
  Link2,
  Mail,
  MessageSquare,
  Music,
  Tv
} from "lucide-react";

// Inline brand SVGs to bypass lucide-react trademark brand deprecations
export const GithubIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

export const YoutubeIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.56 49.56 0 0 1-16.2 0A2 2 0 0 1 2.5 17" />
    <polygon points="10 15 15 12 10 9" />
  </svg>
);

export const TwitterIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
  </svg>
);

export const InstagramIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <rect width="20" height="24" x="2" y="2" rx="5" ry="5" />
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
    <line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
  </svg>
);

export const LinkedinIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect width="4" height="12" x="2" y="9" />
    <circle cx="4" cy="4" r="2" />
  </svg>
);

export const FacebookIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
  </svg>
);

/**
 * Automatically detects the brand/social platform from a URL and returns a suitable SVG icon component
 * and a platform color accent.
 */
export function getSocialBrand(url: string) {
  const lowercaseUrl = url.toLowerCase();
  
  if (lowercaseUrl.includes("github.com")) {
    return {
      name: "GitHub",
      Icon: GithubIcon,
      color: "#24292e"
    };
  }
  if (lowercaseUrl.includes("youtube.com") || lowercaseUrl.includes("youtu.be")) {
    return {
      name: "YouTube",
      Icon: YoutubeIcon,
      color: "#ff0000"
    };
  }
  if (lowercaseUrl.includes("twitter.com") || lowercaseUrl.includes("x.com")) {
    return {
      name: "X / Twitter",
      Icon: TwitterIcon,
      color: "#000000"
    };
  }
  if (lowercaseUrl.includes("instagram.com")) {
    return {
      name: "Instagram",
      Icon: InstagramIcon,
      color: "#e1306c"
    };
  }
  if (lowercaseUrl.includes("linkedin.com")) {
    return {
      name: "LinkedIn",
      Icon: LinkedinIcon,
      color: "#0077b5"
    };
  }
  if (lowercaseUrl.includes("facebook.com")) {
    return {
      name: "Facebook",
      Icon: FacebookIcon,
      color: "#1877f2"
    };
  }
  if (lowercaseUrl.includes("twitch.tv")) {
    return {
      name: "Twitch",
      Icon: Tv,
      color: "#9146ff"
    };
  }
  if (lowercaseUrl.includes("tiktok.com")) {
    return {
      name: "TikTok",
      Icon: Music,
      color: "#010101"
    };
  }
  if (lowercaseUrl.includes("mailto:") || lowercaseUrl.includes("@")) {
    return {
      name: "Email",
      Icon: Mail,
      color: "#ea4335"
    };
  }
  
  // Default fallback link icon
  return {
    name: "Website",
    Icon: Link2,
    color: "#4f46e5"
  };
}

/**
 * Validates usernames: must be between 3 and 15 characters, containing only lowercase letters, numbers, and underscores.
 */
export function validateUsername(username: string): boolean {
  return /^[a-z0-9_]{3,15}$/.test(username);
}

/**
 * Validates that a string is a valid HTTP/HTTPS URL
 */
export function validateUrl(url: string): boolean {
  try {
    const formattedUrl = url.startsWith("http://") || url.startsWith("https://") ? url : `https://${url}`;
    const parsed = new URL(formattedUrl);
    return parsed.protocol === "http:" || parsed.protocol === "https:";
  } catch (_) {
    return false;
  }
}

/**
 * Auto-corrects missing protocols by adding https://
 */
export function cleanUrl(url: string): string {
  if (!url) return "";
  const trimmed = url.trim();
  if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
    return trimmed;
  }
  return `https://${trimmed}`;
}
